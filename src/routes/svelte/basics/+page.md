...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-8 auto-rows-[1fr]">
  <a href="{base}/svelte/basics/component-basics/" class="flex no-underline group h-full not-prose" style="color: inherit;">
    <div class="section-card p-4 cursor-pointer flex flex-col w-full">
      ...
      <p class="text-sm mb-3" style="color: var(--color-card-desc);">Svelteコンポーネントの核となる構造と Runes の基本を学びます。</p>
      <ul class="text-sm space-y-1 flex-grow list-disc pl-5" style="color: var(--color-card-list);">
        ...
        <li><strong>テンプレート式</strong>: 三項演算子・論理 AND・関数呼び出し</li>
        <li><strong>イベント処理</strong>: DOM イベント、修飾子の Svelte 4 → 5 移行</li>
        <li><strong>プロパティ</strong>: <code>$props</code> による親子間のデータ受け渡し</li>
        <li><strong>双方向バインディング</strong>: <code>bind:</code> でのフォーム連携</li>
        <li><strong>スタイリング</strong>: スコープ付き CSS</li>
      </ul>
    </div>
  </a>
  ...
  <a href="{base}/svelte/basics/template-syntax/" class="flex no-underline group h-full not-prose" style="color: inherit;">
    <div class="section-card p-4 cursor-pointer flex flex-col w-full">
      ...
      <p class="text-sm mb-3" style="color: var(--color-card-desc);">Svelte の制御フロー・アノテーション・特殊バインディングを網羅します。</p>
      <ul class="text-sm space-y-1 flex-grow list-disc pl-5" style="color: var(--color-card-list);">
        <li><code>&#123;#if&#125;</code> / <code>&#123;#each&#125;</code> / <code>&#123;#await&#125;</code>: 基本制御フロー</li>
        <li><code>&#123;#key&#125;</code> / <code>&#123;#snippet&#125;</code>: 高度なブロック構文</li>
        <li><code>&#123;@render&#125;</code> / <code>&#123;@html&#125;</code> / <code>&#123;@const&#125;</code> / <code>&#123;@debug&#125;</code>: アノテーション</li>
        <li><code>bind:innerHTML</code> / <code>bind:textContent</code>: 特殊バインディング</li>
      </ul>
    </div>
  </a>
  ...
  <a href="{base}/svelte/basics/special-elements/#svelteboundary---エラーバウンダリ" class="flex no-underline group h-full not-prose" style="color: inherit;">
    <div class="section-card p-4 cursor-pointer flex flex-col w-full">
      <div class="text-3xl mb-2">🛡️</div>
      <h3 class="font-bold text-lg mb-2" style="color: var(--color-card-title);">
        &lt;svelte:boundary&gt; とエラー境界
        <span class="inline-block ml-1 text-xs opacity-60">→</span>
      </h3>
      <p class="text-sm mb-3" style="color: var(--color-card-desc);">Svelte 5.3+ のエラーバウンダリで堅牢な UI を構築します。</p>
      <ul class="text-sm space-y-1 flex-grow list-disc pl-5" style="color: var(--color-card-list);">
        <li><code>failed</code> snippet によるフォールバック UI</li>
        <li><code>pending</code> snippet と await expressions の連携</li>
        <li><code>onerror</code> によるエラー収集</li>
        <li>キャッチ対象とそうでないエラーの境界</li>
        <li>実装例：部分的なエラーリカバリ</li>
      </ul>
    </div>
  </a>
