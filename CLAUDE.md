...
2. **navigation-from-config.ts** (`/src/lib/utils/`)
   - `vite.config.ts`のサイドバー構造からナビゲーション構造を自動生成
   - サイドバー構造をフラットなリストに変換
   - 現在のページの前後を判定する関数を提供
...
1. **単一情報源の原則**: `vite.config.ts`のサイドバー構造を唯一の情報源として使用
...
1. `vite.config.ts`でサイドバー構造を定義
2. AutoPageNavigationコンポーネントが自動的に各ページにナビゲーションを追加
3. 新しいページを追加する際は、`vite.config.ts`のサイドバー構造に追加するだけ
