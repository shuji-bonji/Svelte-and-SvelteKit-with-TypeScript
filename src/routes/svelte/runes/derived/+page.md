...
<ul>
<li><strong><code>$derived(式)</code></strong> - 単純な 1 行の式（<code>count * 2</code>、<code>items.length</code>など）</li>
<li><strong><code>$derived.by(() => &#123; ... &#125;)</code></strong> - 複数ステートメントや複雑なロジック</li>
</ul>
:::
...
<Admonition type="note" title="オーバーライドの動作">

<ul>
<li>オーバーライドされた値は、依存する状態が変更されるまで維持されます</li>
<li>依存状態が変更されると、派生値は再計算されオーバーライドは解除されます</li>
<li>フォーム入力の一時的な上書きなどに便利です</li>
</ul>
:::
