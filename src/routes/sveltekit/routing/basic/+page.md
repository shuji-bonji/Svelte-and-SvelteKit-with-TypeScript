...
<Admonition type="tip" title="重要なルール">

<ul>
<li><code>+page.svelte</code> がないディレクトリはルートとして認識されません</li>
<li><code>+layout.svelte</code> は子ルートに継承されます</li>
<li>ディレクトリ名がそのままURLパスになります</li>
</ul>
...
<Admonition type="info" title="Load関数とは？">
...
HTTPステータスコードに応じて適切なエラーメッセージを表示するカスタムエラーページの実装例です。`$app/state`の`page`オブジェクトからエラー情報を取得し、ユーザーフレンドリーなメッセージを表示します。
...
<Admonition type="tip" title="resolveを使用する理由">

`resolve`を使用することで、アプリケーションがサブディレクトリにデプロイされた場合でも正しく動作します。例えば、`/myapp/`配下にデプロイされた場合、
<ul>
<li><code>href="/about"</code> → ❌ <code>/about</code>（ルートから）</li>
<li><code>href=&#123;resolve('/about')&#125;</code> → ✅ <code>/myapp/about</code>（正しいパス）</li>
</ul>
...
<details>
<summary>ナビゲーションコンポーネントの実装例</summary>
...
<Admonition type="info" title="詳細な設定について">

条件付きプリレンダリング、エッジランタイム設定、キャッシュ制御などの高度な設定については、[高度なルーティング - ルートアノテーション](/sveltekit/routing/advanced/#ルートアノテーション（ページオプション詳細）)で詳しく解説しています。
...
<Admonition type="info" title="完全な実装例を見る">

実際に動作するブログシステムの実装例を2つ用意しています。

<ul>
<li><strong><a href="/examples/blog-system/">ブログシステム（基礎）</a></strong> - 静的データを使った基本的なルーティング実装</li>
<li><strong><a href="/examples/markdown-blog/">Markdownブログ（発展）</a></strong> - 動的ルーティング（<code>[slug]</code>）とMarkdownファイルの自動読み込み</li>
</ul>
...
<Admonition type="warning" title="404エラーが発生する">

<ul>
<li>ファイル名が正しいか確認（<code>+page.svelte</code>であること）</li>
<li>ディレクトリ構造が正しいか確認</li>
<li>開発サーバーを再起動してみる</li>
</ul>

</Admonition>
<Admonition type="tip" title="型エラーの解決">
...
<ul>
<li><strong>ファイルの実行環境</strong>: 各ファイルがどこで実行されるかは<a href="/sveltekit/basics/project-structure/#レンダリングモード別の実行タイミング">プロジェクト構造</a>を参照</li>
<li><strong>レンダリング戦略</strong>: SSR/SSG/SPAの選択は<a href="/sveltekit/basics/rendering-strategies/">レンダリング戦略</a>を参照</li>
<li><strong>特殊ファイルの詳細</strong>: <code>+page</code>、<code>+layout</code>等の詳細は<a href="/sveltekit/basics/file-system/">特殊ファイルシステム</a>を参照</li>
</ul>
