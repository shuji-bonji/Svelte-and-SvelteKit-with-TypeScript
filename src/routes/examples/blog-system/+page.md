...
<li>TypeScriptによる型安全なデータ管理</li>
<li>レイアウトシステムによるUI共通化</li>
<li>ナビゲーションコンポーネントの実装</li>
</ul>
...
const articles: Article[] = [
  {
    ...
    description:
      'SvelteKitを使ったWebアプリ開発の第一歩。プロジェクト作成からルーティングまでを解説します。',
  ...
  },
  {
    ...
    description:
      'Svelte 5で導入されたRunesシステムを、$state・$derived・$effectを中心に解説します。',
  ...
  },
  {
    ...
    description:
      'TypeScriptを用いた実装のベストプラクティスを、型安全性と保守性の観点からまとめます。',
...
<script lang="ts">
  ...
  // href はリテラル union 型にして、resolve() の型付きルーティングが効くようにする
  type NavItem = {
    href: '/' | '/blog' | '/about';
    label: string;
    matchPath?: string;
...
<script lang="ts">
  ...
  // Markdownをレンダリング（marked v16 は async オプションを明示すると string を返す）
  const htmlContent = $derived(marked.parse(data.article.content, { async: false }));
</script>
