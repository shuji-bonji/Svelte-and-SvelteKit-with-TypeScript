...
:::tip[関数ベースの実装]
コレクションの管理を関数ベースで行う場合は、[リアクティブストア](/advanced/reactive-stores/#リアクティブコレクション)でファクトリー関数を使った実装例を紹介しています。クラスベースと関数ベース、それぞれのアプローチには利点があるので、プロジェクトのニーズに合わせて選択してください。
:::
