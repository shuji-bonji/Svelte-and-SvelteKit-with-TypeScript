  ...
  <table>
    ...
    <tbody>
      ...
      <tr class="native">
        ...
        <td><a href="#arrayとobjectネイティブ対応"><code>$state([])</code></a></td>
        <td>配列データの管理</td>
        <td>🔄 ネイティブ対応</td>
      </tr>
      <tr class="native">
...
<script lang="ts">
  ...
  // 型安全なMapの作成（SvelteMap 自体が reactive なので $state ラップは不要）
  let userScores = new SvelteMap<string, number>();
  ...
  function addUser() {
    const name = `ユーザー${userScores.size}`;
    if (name) {
...
<div class="scoreboard">
  ...
  <ul>
    {#each userScores as [name, score] (name)}
      <li>
...
<script lang="ts">
  ...
  // 選択されたタグを管理（SvelteSet 自体が reactive なので $state ラップは不要）
...
<script lang="ts">
  ...
  // 初期パラメータ（SvelteURLSearchParams 自体が reactive なので $state ラップは不要）
  let params = new SvelteURLSearchParams('?category=electronics&sort=price&page=1');
