...
├── application/     # アプリケーション構築（認証・認可、セッション、認証ベストプラクティス、テスト、状態管理、snapshots、環境変数管理、エラーハンドリング、データベース統合）
├── optimization/    # 最適化（ビルド、キャッシュ、PWA、observability、performance、SEO）
└── deployment/      # デプロイ（platforms、execution-environments、packaging、security、monitoring）
```

### 第4部：実装例 (`/examples/`)
ブログシステム（基礎/Markdown）、TODOアプリ、認証システム（Cookie/JWT）、データフェッチング、WebSocket/SSE/リアルタイム通信、Mermaid SSRデモ

### 第5部：リファレンス (`/reference/`)
Svelte 5完全リファレンス、SvelteKit 2.x完全リファレンス

### 第6部：技術詳解 (`/deep-dive/`)
コンパイル時最適化、リアクティブ状態、派生値比較、HTMLテンプレート、素のJS構文、自動生成型、Web Components/CSS戦略、プレースホルダー、動的コンテンツ取り扱い API 比較、Svelte 5 シグナルグラフ内部構造
...
**チェックリスト**（ページ追加・削除・改名・**ステータス変更時**）:

1. `sidebar.ts` の配列を更新
2. 該当するセクション・ハブ（例: `/svelte/runes/+page.md`）のカード一覧を更新（カード見出し・説明文・項目リスト・状態バッジ「⏳ 準備中」の有無もすべて同期）
3. トップレベル・ハブ（`/svelte/+page.md` または `/sveltekit/+page.md`）の **学習ロードマップ Mermaid 図とステップ番号** および **章ごとのリンク一覧表** を更新（「準備中」のスパン表記もリンク化する）
4. ハブ末尾の「学習の進め方」ステップ番号の列挙文も同期
5. `CHANGELOG.md` に変更を記録

> [!CAUTION]
> **「準備中」表記同期の漏れに注意**
>
> ページの実体が完成しても、トップレベル・ハブ（`/sveltekit/+page.md` 等）の表組み・ステップ番号の `（準備中）` 表記が残っているケースがあった（2026-05-16 Sprint 4 で発覚）。記事を骨子フル化したら **必ずトップレベル・ハブも grep して `準備中` の残存を確認** すること。
