---
title: MCP × ESLint 四点セット品質保証
description: Svelte MCP・@eslint/mcp・eslint-plugin-svelte・typescript-eslint の四点セット連携。役割分担、@eslint/mcp によるAI直接リント、フル構成flat config、CI/CD統合、トラブルシューティングを解説。
---
...
<script>
  ...
  const roleDiagram = `flowchart TB
    subgraph ai["AI との対話時"]
        ...
        ESLintMCP["@eslint/mcp<br/>プロジェクト ESLint ルール実行"]
        ESLintMCP -->|lintFile / lintText| LintResult["構造化 lint 結果"]
        ESLintMCP -->|getLintFixed*| FixResult["auto-fix 済みコード"]
    end
    ...
    ESLintMCP -.->|同じ eslint.config.js| Parser
    ...
    style ESLintMCP fill:#7c4dff,stroke:#7c4dff,color:#fff
    style LintResult fill:#7c4dff,stroke:#7c4dff,color:#fff
    style FixResult fill:#7c4dff,stroke:#7c4dff,color:#fff
  ...
  const overlapDiagram = `flowchart LR
    subgraph Overlap["重複領域（ESLint / Svelte MCP 両方が検出）"]
        Legacy["\\$: 反応的宣言"]
        Slot["&lt;slot /&gt; 構文"]
    ...
    subgraph MCPOnly["Svelte MCP のみ検出"]
    ...
    subgraph Bridge["@eslint/mcp のブリッジ"]
        BridgeDesc["AI が ESLint のみ検出領域を<br/>対話中にリアルタイム検証"]
    end
    ...
    style Bridge fill:#ede7f6,stroke:#7c4dff`;

  const ciDiagram = `sequenceDiagram
    participant Dev as 開発者
    participant Editor as エディタ<br/>（VS Code / Cursor）
    participant SvelteMCP as Svelte MCP
    participant ESLintMCP as @eslint/mcp
    participant Git as Git Push
    participant CI as GitHub Actions
    participant ESLint as ESLint

    Note over Dev,ESLintMCP: 開発フェーズ
    Dev->>SvelteMCP: AI にコード生成依頼
    SvelteMCP->>SvelteMCP: svelte-autofixer で検証
    SvelteMCP-->>Dev: Svelte 5 準拠のコード
    Dev->>ESLintMCP: AI にリント依頼
    ESLintMCP->>ESLintMCP: lintText で全ルール検証
    ESLintMCP-->>Dev: 構造化 lint 結果
    Dev->>Editor: コードを編集
...
AI 時代の Svelte 5 開発では、**Svelte MCP**、**@eslint/mcp**、**eslint-plugin-svelte**、**typescript-eslint** の四点セットで品質保証を構築するのが 2026 年時点のベストプラクティスです。それぞれ異なる層で動作し、重複することなく互いを補完します。

2025 年 5 月に ESLint 公式が MCP サーバー `@eslint/mcp` をリリースしたことで、AI エージェントが **プロジェクト固有の ESLint ルールを直接実行** できるようになりました。Svelte MCP の `svelte-autofixer` と組み合わせることで、AI 対話時の品質保証がさらに強固になります。

この記事では、四者の役割分担、`@eslint/mcp` の導入方法、具体的なフル構成 `eslint.config.js`、Claude Code や Cursor との統合方法、CI/CD 連携、よくあるトラブルシューティングを解説します。
...
- Svelte MCP、@eslint/mcp、eslint-plugin-svelte、typescript-eslint の役割分担
- `@eslint/mcp` の導入と Svelte プロジェクトでの活用
- 2026 年時点の安定性と実用判断
...
四つのツールはそれぞれ異なる層で動作します。一言でまとめると：

- **Svelte MCP** は「**AI がコードを書く** 時」の Svelte 5 準拠保証
- **@eslint/mcp** は「**AI がプロジェクトルールを検証する** 時」のブリッジ
...
| 項目 | Svelte MCP | @eslint/mcp | eslint-plugin-svelte | typescript-eslint |
|---|---|---|---|---|
| 主目的 | AI 生成コードの Svelte 5 準拠保証 | AI から ESLint を直接実行 | Svelte 構文の静的解析 | TypeScript 構文の静的解析 |
| 実行タイミング | AI との対話時 | AI との対話時 | 保存時・CI | 保存時・CI |
| 代表的な機能 | `svelte-autofixer`、`get-documentation` | `lintFile`、`lintText`、`getLintFixed*` | 非推奨構文検出、a11y | 型情報ベースのルール |
| 内部で使うパーサ | Svelte コンパイラ | プロジェクトの `eslint.config.js` に依存 | `svelte-eslint-parser` | `@typescript-eslint/parser` |
| 動作する場所 | AI エージェント（Claude Code、Cursor） | AI エージェント（Claude Code、Cursor、Copilot） | エディタ、CI | エディタ、CI |
...
**重複領域**: `$:` 反応的宣言、`<slot />` 構文、`export let` プロパティ、`on:click` など Svelte 4 のレガシー構文は、ESLint と Svelte MCP 両方で検出できます。この領域は「二重の安全網」として機能します。

**ESLint のみ検出**: 未使用変数、`any` の使用、a11y 違反、認知的複雑度、import 順序、命名規則など、汎用的な品質問題は ESLint の独壇場です。

**Svelte MCP のみ検出**: `$state.eager` のような最新ルーン、公式ドキュメントとの整合性、AI 生成時のコンテキスト保持、Svelte 4 → 5 移行支援は Svelte MCP が強みを発揮する領域です。

**@eslint/mcp のブリッジ**: 従来「ESLint のみ検出」だった領域は、保存時か CI でしかチェックできませんでした。`@eslint/mcp` によって、AI エージェントがこの領域を **対話中にリアルタイムで検証** できるようになります。つまり、未使用変数や `any` 使用などの問題が、コードを書いた直後に AI 側で検出・修正されます。
...
| `@eslint/mcp` | 0.3.x | ★★★★☆ | ESLint 公式。v9.26.0 で統合後、独立パッケージ化 |

<Admonition type="info" title="どこまで実務投入できるか">

2026 年時点で、四点セットいずれも **業務プロジェクトへの投入に耐える安定性** に達しています。ただし `eslint-plugin-svelte` は patch リリース頻度が高いため、バージョン固定運用を推奨します（後述）。`@eslint/mcp` はまだ 0.x ですが、ESLint 本体のルール実行を MCP 経由で呼ぶだけのシンプルな構造なので、実用上の問題は少ないです。

</Admonition>

## @eslint/mcp — AI に ESLint を直接使わせる

`@eslint/mcp` は ESLint 公式が提供する MCP サーバーです。AI エージェント（Claude Code、Cursor、GitHub Copilot 等）が **プロジェクトの `eslint.config.js` に基づいた lint をリアルタイムで実行** できるようになります。

### なぜ必要か

Svelte MCP の `svelte-autofixer` は Svelte 5 固有の構文チェックに特化しています。一方、プロジェクト固有のルール（`any` 禁止、未使用変数、import 順序、a11y 等）は `eslint.config.js` で定義されており、従来はエディタ保存時か CI でしかチェックされませんでした。

`@eslint/mcp` を導入すると、AI がコードを生成・修正した **その場で** プロジェクトの全 ESLint ルールを検証できます。

### 提供ツール

`@eslint/mcp` は 4 つの MCP ツールを提供します。

| ツール | 用途 |
|---|---|
| `lintFile` | ファイルパスを渡して lint 実行。構造化 JSON で結果を返す |
| `lintText` | テキストを渡して lint 実行。ファイル保存前の検証に使う |
| `getLintFixedFileContent` | ファイルに auto-fix を適用した結果を返す |
| `getLintFixedTextContent` | テキストに auto-fix を適用した結果を返す |

### セットアップ

`@eslint/mcp` は `npx` で直接起動できます。プロジェクトへのインストールは不要です。
...
```json
{
  "mcpServers": {
    ...
    "svelte": {
      "command": "npx",
      "args": ["-y", "@anthropic-ai/svelte-mcp@latest"]
    }
  }
}
```
...
:::tip[eslint-plugin-svelte との統合は自動]
`@eslint/mcp` はプロジェクトの `eslint.config.js` をそのまま使います。`eslint-plugin-svelte` が設定されていれば、Svelte 固有のルール（a11y、非推奨構文等）も **追加設定なし** で適用されます。
:::

### Svelte MCP との併用ワークフロー

二つの MCP サーバーは異なる専門性を持ちます。AI エージェントは用途に応じて自動的に使い分けます。

```
コード生成 → svelte-autofixer（Svelte 5 構文の準拠チェック）
    ↓
修正後 → @eslint/mcp の lintText（プロジェクト全ルールの検証）
    ↓
問題あり → @eslint/mcp の getLintFixedTextContent（auto-fix 適用）
    ↓
最終確認 → svelte-autofixer（修正後も Svelte 5 準拠か再確認）
```

:::note
`@eslint/mcp` はあくまで ESLint の MCP ラッパーです。新しいルールを追加するものではなく、既存の `eslint.config.js` の検証能力を AI に開放するツールです。したがって、検出能力を向上させたい場合は `eslint.config.js` のルール設定を充実させることが本質的な対策になります。
:::
...
このプロジェクトは以下の四点セットで品質保証しています：

1. **Svelte MCP**: AI にコード生成・修正を依頼する際、必ず `svelte-autofixer` で検証すること
2. **@eslint/mcp**: コード生成後に `lintFile` / `lintText` で ESLint ルール全体を検証すること
3. **eslint-plugin-svelte**: 保存時・CI で自動実行される
4. **typescript-eslint**: `@typescript-eslint/no-explicit-any` は error。any 使用時は必ず理由をコメントで明記

### 優先順位
- Svelte 固有のレガシー構文（`$:`、`<slot />`、`export let`、`on:click`）の検出で Svelte MCP と ESLint が衝突した場合は Svelte MCP を優先
- その他は ESLint のルールを優先（@eslint/mcp で得た結果も同等）
```
...
最後に、四点セットの役割分担を 1 枚にまとめます。

| シチュエーション | 使うツール |
|---|---|
| AI にコード生成を依頼する | Svelte MCP |
| AI にリファクタリングを依頼する | Svelte MCP + get-documentation |
| AI 生成コードをプロジェクトルールで検証 | @eslint/mcp（`lintText` / `lintFile`） |
| AI に auto-fix を適用させる | @eslint/mcp（`getLintFixed*`） |
| Svelte 4 → 5 の移行 | Svelte MCP（`svelte-autofixer` が最強） |
| 未使用変数・import 順序の統一 | ESLint (typescript-eslint) / @eslint/mcp |
| `any` の禁止 | ESLint (typescript-eslint) / @eslint/mcp |
| a11y 違反の検出 | ESLint (eslint-plugin-svelte) / @eslint/mcp |
...
Svelte MCP × @eslint/mcp × eslint-plugin-svelte × typescript-eslint の四点セットは、AI 時代の Svelte 5 開発における品質保証の完成形です。それぞれ異なる層で動作し、冗長性によって互いの弱点をカバーします。

- **Svelte MCP** は AI 対話時の Svelte 5 準拠保証
- **@eslint/mcp** は AI が ESLint ルール全体をリアルタイム検証するブリッジ
...
- **CI での多重チェック** で PR 品質を担保

`@eslint/mcp` の登場により、従来は「保存時・CI まで待つしかなかった」ESLint の検証能力が AI 対話時に前倒しされ、品質フィードバックループが大幅に短縮されました。2026 年時点で四点セット全て実務投入可能な安定性に達しています。
