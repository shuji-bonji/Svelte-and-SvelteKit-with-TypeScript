...
### Sprint 1-D: examples/auth-jwt 状態表記整合

- **`auth-jwt/+page.md`** の「準備中」を「完成」に更新、デモ URL `https://svelte5-auth-jwt.vercel.app/` を反映。
- **`examples/+page.md`** カードの「⏳ 準備中」バッジを「✅ 完成」に変更。
- **`auth-system/+page.md`** の「2025年10月公開予定」表記をデモ／リポジトリへのリンクに変更。学習ロードマップ末尾の「JWT認証（準備中）」も `[JWT認証](/examples/auth-jwt/)` リンクに修正。
