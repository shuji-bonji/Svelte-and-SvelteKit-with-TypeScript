...
- **デモサイト**: 準備中
- **ソースコード**: 準備中（リポジトリ作成予定 `shuji-bonji/svelte5-auth-jwt`）
...
<li><strong>トークンの有効期限</strong> - アクセストークンは短く（15分以内）設定</li>
<li><strong>HTTPS必須</strong> - 本番環境では必ずHTTPS通信を使用</li>
</ol>
...
async function verifyTokenWithCache(
  token: string,
): Promise<TokenPayload | null> {
  // キャッシュチェック
