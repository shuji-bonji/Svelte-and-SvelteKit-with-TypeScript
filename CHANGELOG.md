...
ユーザーからの質問「`{isLoggedIn ? 'Logged In' : 'Logged Out'}` のような **テンプレート式内の三項演算子** は `$state` ルーンの説明で扱っているか？」を受けて確認したところ、`/svelte/` 配下全体で「三項演算子」「条件演算子」の言及がゼロ、テンプレート式での `? :` 使用例も 0 件と判明。これは構文の **位置づけレベル** でのカバー漏れだったため、`### テンプレート構文の基本` セクションの導入を「2 系統の整理」に書き換えた。

### 背景の整理

会話の中で「三項演算子はルーンと一緒に使うのが前提？」という直感的な疑問が出たが、これは **違う** と判明。三項演算子は JavaScript 標準構文であり、Svelte のテンプレート式 `{ ... }` は **任意の JS 式評価エンジン** にすぎない。`$state` などのリアクティブな源を参照する式は値変化で自動再評価されるが、構文レベルでは `$state` と独立した話。

### 変更内容

`src/routes/svelte/basics/component-basics/+page.md` の L282 周辺の `### テンプレート構文の基本` セクションを全面拡充:

- **2 系統の整理を冒頭で明示**:
  1. **式展開 `{ expr }`** — 中身は普通の JS 式として評価
  2. **特殊ブロック構文 `{#if}` / `{#each}` / `{#await}`** — テンプレートの構造を制御する Svelte 独自構文

- **新規 sub セクション `#### 式展開 - JS 式そのままが書ける`** を追加し、live ブロックで 6 種類の式展開パターン（単純値・計算式・**三項演算子**・**論理 AND 短絡評価**・メソッド呼び出し・プロパティアクセス）をインタラクティブに体験可能に
- 「**三項演算子はルーンの相棒ではない**」を本文で明示し、「テンプレート式の評価」と「リアクティビティ」が独立した話であることを構造的に説明
- `:::tip[{#if} と三項演算子の使い分け]` Admonition で「ブロックを丸ごと → `{#if}`」「式 1 つの結果 → 三項演算子」「表示するかどうかだけ → 短絡評価 `{flag && ...}`」の判断軸を表にして提示
- 既存の `<Admonition type="tip" title="より高度なテンプレート構文">` はそのまま維持し、その後に「ここから先はブロック構文を順に見ていく」と橋渡し文を追加

### 当初案からの方針変更

最初は「`### 条件分岐 - ifブロック` の末尾に三項演算子の Admonition を追加」案を検討したが、ユーザーとの対話で「三項演算子は `{#if}` の補足ではなく **テンプレート式という一段上のレイヤ** の話」と気づき、構造を変更。Admonition で補足するより、`### テンプレート構文の基本` の冒頭で **2 系統の存在** を最初に提示し、その内訳として式展開（三項演算子含む）とブロック構文を並べる構成のほうが、学習者の理解モデルとして正確になる。

### 検証

- `svelte-autofixer`（新規 live 例）: issues 0 件
- `npm run lint:articles`: errors=4 / warnings=36（ベースライン維持）、blocks 680 → 681
