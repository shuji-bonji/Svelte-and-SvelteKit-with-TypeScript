...
SvelteKit のデフォルトトレーシングでは、基本的なリクエスト情報のみが記録されます。`event.tracing` を使用してカスタム属性を追加することで、ビジネスロジックに関連する情報（ユーザーID、処理件数、エラー詳細など）も記録できます。

### root スパンと current スパン

SvelteKit では、2種類のスパンを使い分けることができます。`root` スパンはリクエスト全体に関連する情報（ユーザー情報など）を記録し、`current` スパンは現在の処理に固有の情報を記録します。
...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  event.tracing.current.setAttribute('dashboard.activities.count', activities.length);
