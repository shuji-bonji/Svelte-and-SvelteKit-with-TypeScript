...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-8 auto-rows-[1fr]">
  ...
  <a href="{base}/deep-dive/dynamic-content-api-comparison/" class="flex no-underline group h-full not-prose" style="color: inherit;">
    <div class="section-card p-4 cursor-pointer flex flex-col w-full">
      <div class="text-3xl mb-2">🔀</div>
      <h3 class="font-bold text-lg mb-2" style="color: var(--color-card-title);">
        動的コンテンツ取り扱い API の比較
      ...
      <p class="text-sm mb-3" style="color: var(--color-card-desc);"><code>bind:innerHTML</code>、<code>&#123;@html&#125;</code>、<code>&#123;#snippet&#125;</code>/<code>&#123;@render&#125;</code>、<code>children</code> snippet の 4 つの動的コンテンツ API を横並びで比較し、使い分けの判断フローを解説します。</p>
      <ul class="text-sm space-y-1 flex-grow list-disc pl-5" style="color: var(--color-card-list);">
        <li>双方向 vs 一方向、DOM API vs 言語機能の整理</li>
        <li>XSS リスクと型安全性の違い</li>
        <li>判断フロー Mermaid 図と Svelte 4→5 マッピング</li>
      </ul>
    </div>
  </a>

  <a href="{base}/deep-dive/svelte-signal-graph/" class="flex no-underline group h-full not-prose" style="color: inherit;">
    <div class="section-card p-4 cursor-pointer flex flex-col w-full">
      <div class="text-3xl mb-2">🕸️</div>
      <h3 class="font-bold text-lg mb-2" style="color: var(--color-card-title);">
        Svelte 5 シグナルグラフの内部構造
        <span class="inline-block ml-1 text-xs opacity-60">→</span>
      </h3>
      <p class="text-sm mb-3" style="color: var(--color-card-desc);">Svelte 5 のシグナルベース fine-grained reactivity の内部実装を解剖。Source/Reaction ノード、依存追跡、スケジューリングを TypeScript で読み解きます。</p>
      <ul class="text-sm space-y-1 flex-grow list-disc pl-5" style="color: var(--color-card-list);">
        <li>Source（$state）と Reaction（$derived/$effect）の構造</li>
        <li>自己申告的トラッキングと動的依存解決</li>
        <li>マイクロタスクスケジューラと React/Solid との比較</li>
      </ul>
    </div>
  </a>
</div>
