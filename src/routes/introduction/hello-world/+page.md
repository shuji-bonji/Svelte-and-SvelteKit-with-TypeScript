...
TypeScriptを使用する場合は、`&lt;script lang="ts"&gt;`を指定します。型注釈を追加することで、より安全なコードを書くことができます。
...
TypeScriptは初期値から型を推論できるため、明らかな場合は型注釈を省略することもできます。ただし、明示的な型注釈はコードの意図を明確にし、チーム開発では特に有用です。
</Admonition>
...
<Admonition type="tip" title="違いのポイント">
<ul>
<li><strong>従来</strong>: <code>let</code>で宣言した変数が自動的にリアクティブ</li>
<li><strong>Svelte 5</strong>: <code>$state()</code>で明示的にリアクティブな状態を宣言</li>
<li><strong>イベント</strong>: <code>on:click</code>から<code>onclick</code>へ（より標準的なHTML構文に）</li>
</ul>
