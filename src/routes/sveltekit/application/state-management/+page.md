...
:::warning[SSR で複数リクエストの状態が混ざります]
**`.svelte.ts` のモジュールトップレベルで `$state` を生成して `export const` するパターンは、SvelteKit の SSR では危険なアンチパターン**です。

```typescript
// アンチパターン
class CartStore { items = $state<CartItem[]>([]); }
export const cart = new CartStore(); // Node.js プロセスで全リクエストが共有
```

サーバー（Node.js）は1つのプロセスを多数のユーザーで共有します。モジュールトップレベルで生成した `$state` はリクエスト間で共有され、**ユーザー A の認証情報やショッピングカートがユーザー B に漏洩する可能性**があります。`svelte/store` の `writable` も同様です。

公式ドキュメントでは「[サーバー上で共有状態を持たないこと](https://svelte.jp/docs/kit/state-management#Avoid-shared-state-on-the-server)」が明示されています。

**安全な書き方は以下のいずれか**

- **Context API（`setContext` / `getContext`）**：`+layout.svelte` でリクエストごとに `new` して `setContext`、子コンポーネントで `getContext`
- **コンポーネントローカル `$state`**：`+page.svelte` / `+layout.svelte` の `&lt;script&gt;` 内で `$state` を作る
- **Load 関数 / Remote Functions**：リクエストスコープな関数の戻り値として扱う
- **クライアント限定で割り切る**：`if (browser)` ガードで初期化、SSR では値を持たない

以降のコード例はこの方針に従って書き換えています。
:::

### .svelte.tsファイルによるストア（クラス定義）

以下は、ECサイトのショッピングカート機能を実装した例です。クラスベースのストアパターンを使用することで、ビジネスロジックをカプセル化し、型安全な状態管理を実現しています。

**ポイント**：クラス定義と Context のヘルパーだけを export し、**インスタンスはモジュールトップで `new` しない**（SSR でリクエスト間共有になるため）。
...
export class CartStore {
  // $state はインスタンスごとに作られるためリクエストスコープで安全
...
// Context API でリクエストスコープに閉じ込めるためのヘルパー
...
export function getCartContext(): CartStore {
  ...
  if (!cart) {
    throw new Error(
      'CartStore context not found. 親レイアウトで setCartContext() を呼んでください'
    );
  }
  return cart;
}
...
### 使用例：レイアウトで provide、ページで consume

`+layout.svelte` 側でリクエストごとに `new CartStore()` し、Context に格納します。**ここがリクエストスコープの境界**になります。
...
<script lang="ts">
  ...
  // このコンポーネントツリーに対する CartStore を1つ作る
  // SSR の場合はリクエストごとに、CSR の場合は最初のマウント時に1回だけ実行される
  setCartContext();
</script>
...
子コンポーネントでは `getCartContext()` で取り出して、通常のオブジェクトとして扱えます。
...
<script lang="ts">
  ...
  function addToCart() {
    cart.addItem({ id: '1', name: 'TypeScript Book', price: 2980 });
  }
</script>
...
:::tip[クライアント限定で使いたい場合の割り切り]
ショッピングカートのように **「そもそも SSR で初期化する必要がない」** ケースでは、`$lib/state/cart.svelte.ts` で `export const cart = new CartStore()` してしまい、**サーバー側からは絶対に書き込まないと決め切る**という選択肢もあります。ただしこれは「サーバー側で `cart` を import するコード（`+page.server.ts`、`hooks.server.ts` など）を絶対に書かない」という運用ルールが守れる前提です。チームが大きくなるほど Context API のほうが安全です。
:::
...
// 注意: モジュールトップで new せず、Context API か layout 内で生成する
// （SSR でリクエスト間共有になるため。詳細は冒頭の警告ボックスを参照）
// 例: setContext('editor', new EditorStore()) を +layout.svelte 内で呼ぶ
```
...
// 注意: authMachine は「ユーザー固有」の状態であり、絶対に
// モジュールトップで new してはいけない（SSR で全リクエストに漏洩する）
// 必ず +layout.svelte 内で setContext('auth', new AuthMachine()) する
```
...
以下の例では、フィルター条件をURLのクエリパラメータと同期させるストアを実装しています。`$app/state`の`page`オブジェクトと`goto`関数を活用することで、URLとアプリケーション状態の双方向同期を実現しています。
...
// FilterStore は URL に依存するため、CSR で初期化すれば問題ないが、
// 本格運用では setContext で +layout.svelte 内に閉じ込めるのが推奨
```
...
describe('CartStore', () => {
  ...
  // テストごとに新しいインスタンスを作る = 本番でも同じ単位（リクエスト）で作る
  beforeEach(() => {
    cart = new CartStore();
  });
...
#### 重要なポイント
- **SSR セーフネスを最優先**: モジュールトップで `$state` を生成・export しない。Context API か load 関数経由でリクエストスコープに閉じ込める
- **適切なスコープの選択**: ローカル状態、Context、URL、サーバー状態を使い分ける
- **型安全性の確保**: TypeScriptを活用して、コンパイル時にエラーを検出
- **パフォーマンスの考慮**: 必要に応じてメモ化とキャッシュを実装
- **テストの実施**: ビジネスロジックを単体テストで検証（各テストで `new` し直す）
