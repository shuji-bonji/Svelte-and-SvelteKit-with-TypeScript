...
```typescript
// src/lib/tauri.ts
// Tauri 2.x: invoke は @tauri-apps/api/core からインポート
import { invoke } from '@tauri-apps/api/core';
// Tauri 2.x: dialog / fs / updater 等は個別のプラグインに分離された
...
:::info[Tauri 2.x への移行ポイント]
本ガイドは **Tauri 2.x**（2024-10 GA、現行メジャー）を前提としています。Tauri 1.x との主な API 差分:

- `@tauri-apps/api/tauri` → **`@tauri-apps/api/core`**（`invoke` の import 元）
- `@tauri-apps/api/dialog` → **`@tauri-apps/plugin-dialog`**（プラグイン化）
...
- Tauri 環境判定は `import { isTauri } from '@tauri-apps/api/core'` または `'__TAURI_INTERNALS__' in window` を推奨

各プラグインは `src-tauri/Cargo.toml` への依存追加とパーミッション設定が必要です（[Tauri 2.x ドキュメント](https://v2.tauri.app/) 参照）。
:::
...
// Capacitor 4 以降: @capacitor/storage は @capacitor/preferences に改名された
...
// ローカルストレージ（旧 @capacitor/storage の Storage は Preferences に移行）
...
:::info[Capacitor の `Storage` は `Preferences` に改名]
Capacitor 4.x から `@capacitor/storage` プラグインは **`@capacitor/preferences` に改名**されました。旧パッケージは非推奨となっており、新規プロジェクトでは `@capacitor/preferences` を使ってください。本ガイドは Capacitor 6 系（2024 リリース）以降を前提にしています。
:::
...
```typescript
// Tauri 2.x 自動アップデート（@tauri-apps/plugin-updater）
