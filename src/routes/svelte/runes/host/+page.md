...
```svelte bad
<!-- Counter.svelte（Web Component 化サンプル：initial を初期値スナップショットとして
     内部 state にコピーするため state_referenced_locally 警告が出るが、Web Components
     の attribute は基本的に初回のみ反映するパターンなので意図的） -->
<svelte:options customElement="my-counter" />
...
```svelte bad
<!-- src/lib/components/MyCounter.svelte（Web Component 化サンプル：上記と同様、
     attribute の初期値スナップショットパターンのため state_referenced_locally 許容） -->
<svelte:options customElement="my-counter" />
