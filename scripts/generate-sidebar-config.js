...
async function extractSidebarConfig() {
  try {
    // Read vite.config.ts
    ...
    // This is a simplified extraction - in production, you might want to use a proper TypeScript parser
    const sidebarMatch = viteConfigContent.match(/sidebar:\s*{[\s\S]*?^        \]/m);
    
    if (!sidebarMatch) {
      console.error('Could not find sidebar configuration in vite.config.ts');
      process.exit(1);
    }
    
    // For now, we'll manually export the sidebar config as it's complex to parse TypeScript
    // In a real implementation, you'd use a TypeScript parser like @typescript-eslint/parser
    
    // Generate the sidebar config module
    const configModule = `// This file is auto-generated by scripts/generate-sidebar-config.js
// DO NOT EDIT MANUALLY - Edit vite.config.ts instead

export const generatedSidebarConfig = ${JSON.stringify(extractedConfig, null, 2)};
`;
    
    const outputPath = join(projectRoot, 'src/lib/generated/sidebar-config.json');
    await writeFile(outputPath, JSON.stringify(extractedConfig, null, 2));
...
// This would be extracted from vite.config.ts in a real implementation
