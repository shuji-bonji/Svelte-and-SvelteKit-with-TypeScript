...
<ul>
<li>📊 <strong>現在のページ</strong>: データフローの詳細（初級〜中級）</li>
<li>⚡ <a href="{base}/sveltekit/data-loading/strategies/">データフェッチング戦略</a> - 実践的なパターンとベストプラクティス（中級〜上級）</li>
<li>🏗️ <a href="{base}/sveltekit/architecture/data-loading/">データロードアーキテクチャ</a> - 内部実装とメカニズムの詳解（上級）</li>
</ul>
...
<!-- false: プリフェッチを無効化 -->
<a href="/large-page" data-sveltekit-preload-data="false">
  大きなページへ
</a>
```

:::info[`data-sveltekit-preload-data` の有効値]
`"hover"` / `"tap"` / `"false"` の 3 種類のみです。`"eager"` / `"viewport"` は `data-sveltekit-preload-code` 側で利用可能な値であり、`preload-data` には指定できません。`"off"` も存在しません。詳しくは [link-options](/sveltekit/routing/link-options/) を参照してください。
:::

プログラマティックにプリフェッチを制御することも可能です。SvelteKit 2 では `prefetch()` / `prefetchRoutes()` は **`preloadData()` / `preloadCode()` に置き換わりました**。

```typescript
import { preloadCode, preloadData } from '$app/navigation';

// 単一ページのデータとコードを事前読み込み（load も実行される）
await preloadData('/posts/123');

// 複数ページのコードのみ事前読み込み（モジュール先読み、load は実行されない）
await preloadCode('/about', '/contact', '/products');
```
...
<li>初回表示が遅くなる（JavaScriptのダウンロード・実行を待つ必要）</li>
<li>SEOに大きな影響（検索エンジンはコンテンツを認識できない）</li>
<li>機密情報の扱いに注意（すべてがクライアントコードに含まれる）</li>
</ul>
...
export const load: PageServerLoad = async ({ fetch }) => {
  ...
  // 3. コメント取得（1秒）- 投稿取得を待つ
  const comments = await fetch(`/api/users/${user.id}/comments`).then((r) =>
    r.json(),
  );
...
CSR/SPAモードでのデータフローとデータ無効化の詳細については、[SPAモードとデータ無効化](/sveltekit/data-loading/spa-invalidation/)の専用ページで解説しています。
...
- [データフェッチング戦略](/sveltekit/data-loading/strategies/) - より高度な最適化技術
- [SPAモードとデータ無効化](/sveltekit/data-loading/spa-invalidation/) - SPAモードの詳細とデータ更新
- [ストリーミングSSR](/sveltekit/data-loading/streaming/) - 段階的なコンテンツ配信
