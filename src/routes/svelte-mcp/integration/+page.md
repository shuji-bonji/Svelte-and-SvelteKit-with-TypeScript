...
<Admonition type="info" title="詳細な統合ガイド">
Svelte MCP、eslint-plugin-svelte、typescript-eslint の三点セットによる本格的な品質保証構成、プロジェクト固有のルールカスタマイズ、CI/CD 統合、トラブルシューティングは専用ページ [Svelte MCP × ESLint × typescript-eslint 連携ガイド](/svelte-mcp/eslint-integration/) で詳しく解説しています。本セクションは最小構成の紹介にとどめます。
</Admonition>
...
Svelte 5 では `parserOptions.svelteConfig` に `svelte.config.js` の内容を渡すことで、Runes を正しく認識します。詳細は [ESLint × typescript-eslint 連携ガイド](/svelte-mcp/eslint-integration/) を参照してください。
</Admonition>
...
<Admonition type="tip" title="svelte-task プロンプト">
