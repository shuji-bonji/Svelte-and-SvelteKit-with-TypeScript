...
:::tip[pushState vs replaceState]
- **pushState**: 新しい履歴エントリを作成（「戻る」で元に戻れる）
- **replaceState**: 現在の履歴エントリを置き換え（「戻る」で前のページに戻る）
:::
...
```typescript
// src/app.d.ts
import type { PageData as PhotoPageData } from './routes/photos/[id]/$types';

declare global {
  namespace App {
    interface PageState {
      selected?: PhotoPageData;
    }
  }
}
