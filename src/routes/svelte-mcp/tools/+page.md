...
```typescript
// 単一セクション
get-documentation({ section: 'svelte/runes/state' });

// 複数セクション
get-documentation({
  section: ['svelte/runes/state', 'svelte/runes/derived', 'svelte/runes/effect']
});
```

タイトル（例：`$state`）でも、ファイルパス（例：`svelte/runes/state`）でも検索できます。複数指定した場合はまとめて結果が返ります。
...
{
  "issues": [
    "`$:` is not allowed in runes mode, use `$derived` or `$effect` instead\nhttps://svelte.dev/e/legacy_reactive_statement_invalid at line 3, column 2",
    "`on:click` is deprecated, use the standard DOM event `onclick` instead at line 6, column 8"
  ],
  "suggestions": [
    "let count = 0 を let count = $state(0) に変更することを推奨"
  ],
  "require_another_tool_call_after_fixing": true
}
```

#### `require_another_tool_call_after_fixing` フィールド

`svelte-autofixer` は、検出した `issues` を修正した後に**再度ツール呼び出しが必要かどうか**を示す真偽値フラグ `require_another_tool_call_after_fixing` を返します。

| 値      | 意味                                                                                            |
| ------- | ----------------------------------------------------------------------------------------------- |
| `true`  | 修正を適用した後、再度 `svelte-autofixer` を呼び出して残存する問題がないかを検証する必要がある |
| `false` | これ以上の修正サイクルは不要                                                                    |

このフラグにより、LLM はエージェント的なループの中で「いつ修正サイクルを停止すべきか」を機械的に判断でき、無限ループや過剰なツール呼び出しを避けられます。

### ポイント

- **`require_another_tool_call_after_fixing` が `false` になるまで繰り返し呼び出す**ことを推奨
- LLM は `issues` に基づいてコードを修正し、フラグが `true` の間は再度 `svelte-autofixer` を呼び出す
...
```
https://svelte.dev/playground#H4sIAAAA...（gzip + base64url で圧縮されたコード）
```

生成された URL を開くとコードが Playground に展開されます。URL ハッシュ自体にコードが格納されているため、サーバー側にはコードが保存されません。
