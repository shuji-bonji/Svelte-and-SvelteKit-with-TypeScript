...
description: SvelteKit 2.xのルートグループを使った認証境界の実装例。public/secureでのガード、layoutごとのセッションチェック、型安全なフックとフォーム処理、メンテナブルなディレクトリ設計をまとめたハンズオンチュートリアル。コード全文と動作確認手順付き。詳しい手順とチェックリスト付き
---
...
<Admonition type="info" title="計画中">
...
<script lang="ts">
  ...
  // LayoutProps は data: LayoutData と children: Snippet を含む（SvelteKit 2.16+）
...
<Admonition type="tip" title="実装のヒント">
...
_完全な実装例は2025年11月頃に公開予定です。それまでは[Cookie/Session認証](/examples/auth-cookie-session/)の実装例を参考にしてください。_
