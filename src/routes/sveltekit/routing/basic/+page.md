...
:::tip[resolveRouteを使用する理由]
`resolveRoute`を使用することで、アプリケーションがサブディレクトリにデプロイされた場合でも正しく動作します。例えば、`/myapp/`配下にデプロイされた場合、
- `href="/about"` → ❌ `/about`（ルートから）
- `href={resolveRoute('/about')}` → ✅ `/myapp/about`（正しいパス）
...
<script lang="ts">
  ...
  // resolveRouteを使用してbase pathを考慮したURLを生成
