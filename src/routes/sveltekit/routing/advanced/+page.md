---
title: 高度なルーティング
description: SvelteKitの高度なルーティングをTypeScriptで実装 - ルートグループ、パラメータマッチャー、フォールバック、マッチング優先順位、動的ルート、catch-all、複雑な構造設計を実例を交えて実践的に詳しく解説します
---
...
<script>
  ...
  const errorPropagationDiagram = `graph TB
    subgraph "エラー伝播の階層"
      direction TB
      PageError["/app/posts/[id]/+page.server.ts<br/>error(404)"]
...
<Admonition type="tip" title="レイアウトのリセット">
...
<Admonition type="info" title="SPA出身者向け：SvelteKitのナビゲーション動作">
...
<ul>
<li>ページ全体のリロードは発生しません</li>
<li>ブラウザのURLは変化します</li>
<li>必要な部分のみが更新されます（仮想DOMではなく、Svelteのコンパイラによる効率的な更新）</li>
</ul>
...
<ul>
<li><strong>初回アクセス</strong>: SEOとパフォーマンスのためにSSR/SSG</li>
<li><strong>その後のナビゲーション</strong>: SPAのような高速な遷移</li>
</ul>
...
ページデータ・コードを事前に読み込むことで、実際の遷移時の表示を高速化する機能です。重要なページやユーザーが次に訪れる可能性の高いページを先読みすることで、体感速度を向上させます。

:::info[SvelteKit 2 での API 変更]
SvelteKit 1 系の `prefetch()` / `prefetchRoutes()` は **`preloadData()` / `preloadCode()` に置き換わりました**。`preloadData` は単一 URL の `load` 関数を実行し、`preloadCode` は複数の pathname のモジュールをロード（`load` は実行しない）します。
:::

プリフェッチの実装例です。`onMount` で重要な3つのページ（ダッシュボード、プロフィール、設定）のモジュールを一括で事前読み込みし、初回アクセス時の表示を高速化します。リンクのマウスオーバー時に `preloadData()` を呼び出すことで、クリック前にページデータの読み込みを開始し、瞬時にページ遷移できるようにしています。
...
<script lang="ts">
  ...
  onMount(async () => {
    // 重要なページのコードを事前読み込み（load は実行されない）
    await preloadCode(
      '/dashboard',
      '/profile',
      '/settings'
  ...
  async function preloadNextPage() {
    // 次のページのデータとコードを事前読み込み（load も実行）
    await preloadData('/posts/next-article');
  }
</script>
...
<a
  href="/important-page"
  onmouseenter={() => preloadData('/important-page')}
>
  重要なページ
</a>
```

`<a>` 要素の場合は `data-sveltekit-preload-data="hover"` / `"tap"` 属性で宣言的に同じ挙動を得られます（[link-options](/sveltekit/routing/link-options/) を参照）。
...
export const load: PageServerLoad = async ({ params, locals }) => {
  try {
    ...
    if (!post.published && !locals.user?.isAdmin) {
      error(403, 'This post is not published');
    }
...
<Admonition type="note" title="「並列ルート」ではない理由">
...
リンクごとにプリロード戦略を設定する例です。`data-sveltekit-preload-data` は `hover`（マウスオーバー時、デフォルト）／`tap`（タップ時、モバイルのデータ通信量を節約）／`false`（無効化）の 3 値、`data-sveltekit-preload-code` は `eager`（即座に）／`viewport`（ビューポート内）／`hover`／`tap`／`false` の 5 値を取ります。「データ」と「コード」で許可される値が異なる点に注意してください。

```svelte
<!-- プリロード戦略 -->
<nav>
  <!-- 表示時に「コード」を即座にプリロード（モジュール先読み、load は実行されない） -->
  <a href="/critical" data-sveltekit-preload-code="eager">
    重要なページ
  </a>

  <!-- ホバー時にデータをプリロード（デフォルト） -->
  <a href="/normal" data-sveltekit-preload-data="hover">
    通常のページ
  </a>

  <!-- タップ時にデータをプリロード（モバイル向け） -->
  <a href="/mobile" data-sveltekit-preload-data="tap">
    モバイル向けページ
  </a>

  <!-- データのプリロードを無効化 -->
  <a href="/heavy" data-sveltekit-preload-data="false">
    重いページ
  </a>
</nav>
```

:::info[`preload-data` と `preload-code` で値が異なる]
`data-sveltekit-preload-data` の有効値は `hover` / `tap` / `false` の 3 種類のみ。`eager` / `viewport` を渡すことはできません。コードだけ即座に先読みしてデータは hover/tap で読みたい場合は `data-sveltekit-preload-code` 側を使ってください（[link-options](/sveltekit/routing/link-options/) を参照）。
:::
