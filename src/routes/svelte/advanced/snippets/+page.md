...
```svelte bad
<!-- 学習用断片：本来 {#snippet children()} は不要（子要素を直接書けばよい）が、
     ここでは「複数 snippet を組み合わせる構造の例」として明示的に書いている -->
<!-- 親コンポーネント -->
<script lang="ts">
...
<div class="data-points">
  {#each processedData as data (data.label)}
...
{#snippet genericList(props: ListProps<Task>)}
  {#if props.items.length > 0}
    {#each props.items as item, i (item.id)}
