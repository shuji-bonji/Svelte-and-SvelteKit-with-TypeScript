...
<Admonition type="tip" title="ファイル選択のガイドライン">
<ul>
<li><strong>ページデータが必要</strong> → <code>+page.server.ts</code></li>
<li><strong>複数ページで共通のデータ</strong> → <code>+layout.server.ts</code></li>
<li><strong>JSON APIが必要</strong> → <code>+server.ts</code></li>
<li><strong>すべてのリクエストで実行</strong> → <code>hooks.server.ts</code></li>
</ul>
...
<Admonition type="warning" title="重要なセキュリティ原則">
<ul>
<li><strong>機密情報</strong>（APIキー、DBパスワード等）は<code>*.server.ts</code>または<code>$lib/server/</code>に配置</li>
<li><strong>環境変数</strong>は<code>$env/static/private</code>を使用（<code>process.env</code>も可）</li>
<li><strong>クライアントに送信するデータ</strong>は必要最小限に絞る</li>
</ul>
