...
:::caution[`$props<T>()` ジェネリック呼び出しは非推奨]
公式ドキュメントの推奨パターンは **型を分割代入の右側で `: Props` として渡す**形です（`let { ... }: Props = $props()`）。`$props<T>()` のジェネリック呼び出し構文は混乱を招くため、本プロジェクト全体で `let { ... }: Props = $props()` に統一しています。
:::
...
汎用的なコンポーネントを作成する際に、ジェネリック型を使用して柔軟性を保ちます。`<script>` タグに **`generics="T"` 属性を必ず指定**して、コンポーネントレベルの型パラメータ `T` を導入します。
...
:::warning[`generics="T"` 属性を忘れない]
ジェネリックコンポーネントでは `<script lang="ts" generics="T">` 属性が必須です。これがないと `T` が未定義のままで型エラーになります。`generics="T, U extends string"` のように複数・制約付きの宣言も可能です。
:::
...
ジェネリック型を活用した、汎用的なデータテーブルコンポーネントの実装例です。`<script>` タグに **`generics="T"` 属性**を必ず指定します。`Set<T>` も `svelte/reactivity` の `SvelteSet` を使ってリアクティブにしています。
...
<script lang="ts" generics="T">
  ...
  // SvelteSet で選択状態をリアクティブに（new Set() を $state でラップしても add/delete が検知されない）
  const selected = new SvelteSet<T>();
  let sortColumn = $state<keyof T | null>(null);
  let sortDirection = $state<'asc' | 'desc'>('asc');

  // ソート済みデータ（複雑なロジックには $derived.by を使用）
