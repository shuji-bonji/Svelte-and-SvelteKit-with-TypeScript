...
Remote Functions を使用するには、`svelte.config.js` で機能を有効化します。
...
`query` は、サーバーから動的データを取得するための関数です。
...
引数を受け取るクエリでは、[Standard Schema](https://standardschema.dev/)（Zod や Valibot）でバリデーションを行います。
...
`query.batch` は、同じマクロタスク内の複数のリクエストをまとめて実行します。
...
`form` は、フォームの送信とバリデーションを簡潔に処理するための関数です。
...
サーバー側で追加のバリデーションを行う場合は、`invalid` 関数を使用します。
...
`enhance` メソッドでフォーム送信の動作をカスタマイズできます。
...
`command` は、フォーム以外の方法でサーバーにデータを書き込むための関数です。
...
`withOverride` を使用して、サーバー応答を待たずに UI を更新できます。
...
`prerender` は、ビルド時にデータを取得して静的化するための関数です。
...
Remote Functions 内では `getRequestEvent()` を使用して、現在のリクエスト情報にアクセスできます。
