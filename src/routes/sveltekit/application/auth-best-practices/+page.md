...
<Admonition type="info" title="Rich Harrisの見解">
...
const isAdmin = createGuard(
  (event) => event.locals.user?.roles?.includes('admin') ?? false,
);
...
export class AuthPolicy {
  ...
  static async canManageUsers(user: User): Promise<boolean> {
    return user.roles.includes('admin') || user.roles.includes('user-manager');
  }
}
...
// +page.svelte
<script>
  import {user} from '$lib/stores'; import {goto} from '$app/navigation'; //
  ブラウザのDevToolsで簡単に回避可能 if (!$user) goto('/login');
</script>
```
...
<Admonition type="tip" title="移行のコツ">
